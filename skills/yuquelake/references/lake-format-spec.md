...
**SOP 使用样例**：工作周报用 `<h2>` 划分"本周重点""下周计划""思考"，用 `<h3>` 细分"任务进展""相关数据""风险同步"。
...
**SOP 使用样例**：工作周报中"（↑20%）"用红色文字标强调增长；团建活动文档中酒店描述用灰色文字。
...
> 需要带颜色的提示框样式时用 `<alert>`（见 card-reference.md）。

**SOP 使用样例**：几乎所有 SOP 文档模板都用 `<blockquote>` 在章节标题下添加说明文字，如"在此填写去程和返程的具体时间"。
...
**SOP 使用样例**：团建活动文档中的外部链接；需求文档中引用外部 PRD 链接。
...
**SOP 使用样例**：项目立项文档的"痛点"列表；团建活动的"酒店特点"列表；系分文档的"知识库问题"列表。
...
**SOP 使用样例**：退款 SOP 的参数对照表；工作周报的 UV 数据对比表；用研报告的基本信息表（带单元格背景色和合并）；团建活动的时间安排表。
...
> 永远不要手动编码。使用伪标签 + 转换脚本。详见 [card-reference.md](card-reference.md)。
...
3. `data-lake-id`、`id` 属性和 `<span>` 文本包裹由转换脚本自动生成
